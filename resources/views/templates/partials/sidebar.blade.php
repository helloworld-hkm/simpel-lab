<aside id="sidebar" class="sidebar">
    ...
    <ul class="sidebar-nav" id="sidebar-nav">
        ...
        @can('maintenanceRepair')
            ...
            <li class="nav-item">
                <a class="nav-link {{ $active === 'perbaikan' ? ' ' : 'collapsed' }} d-flex justify-content-between  "
                    href="/perbaikan">
                    <div>
                    ...
                    @if (auth()->user()->role->id == '4' || auth()->user()->role->id == '2')
                        @if ($perbaikanCountTKJT)
                            <span class="mx-2 badge bg-primary text-white"> {{ $perbaikanCountTKJT }} </span>
                        @endif
                        @else
                        @if ($perbaikanCount)
