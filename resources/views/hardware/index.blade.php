    ...
    <div class="modal fade" id="tambahModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form class=" needs-validation" action="/hardware" method="POST" novalidate>
                    <div class="modal-body">
                        @csrf
                        <div class="form-floating mb-3 has-validation">
                            <input type="text" class="form-control" name="hardware" maxlength="255" id="floatingName"
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.hapus-data').on('click', function() {
                ...
                $('#form-hapus').attr('action', 'hardware/' + id);
            });
            $('body').on('click', '.edit-data', function(event) {
                ...
                var id = $(this).data('id');
                $.get('/hardware/' + id + '/edit', function(data) {
                    // $('#modal-student-edit').modal('show');
                    console.log(data);
                    $('#nama-edit').val(data.hardware.hardware);


                    $('#form-edit').attr('action', 'hardware/' + data.hardware.id);
                        ...
                        <tbody>
                            ...
                            @foreach ($data as $lab)
                                ...
                                <tr>
                                    ...
                                    <td > {{ $lab->nama_lab }}</td>
                                    <td>
                                        @if ($lab->hardware->isEmpty())
                                        <div>-</div>
                                        @endif
                                    @foreach ($lab->hardware as $hardware )
                                    <div class="d-flex justify-content-between my-2">{{ $hardware->hardware }}
                                        <span class="float-right">
                                            <button type="button" class="btn btn-success edit-data btn-sm"
                                            data-bs-target="#editModal" data-lab="{{ $lab->id }}"
                                            data-id="{{ $hardware->id }}" data-bs-toggle="modal"><i
                                            class="bi bi-pencil-square"></i> </button>

                                            <button type="button" class="btn btn-danger hapus-data btn-sm"
                                            data-bs-target="#hapusModal" data-id="{{ $hardware->id }}"
                                            data-nama="{{ $hardware->hardware }}" data-bs-toggle="modal"><i
