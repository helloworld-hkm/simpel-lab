...
@push('modal-action')
    ...
    <div class="modal fade" id="tambahModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Tambah Data software </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form class=" needs-validation" action="/software" method="POST" novalidate>
                    <div class="modal-body">
                        @csrf
                        <div class="form-floating mb-3 has-validation">
                            <input type="text" class="form-control" name="software" maxlength="255" id="floatingName"
                                placeholder="Your Name" required>
                            <label for="floatingName">software</label>
    ...
    <div class="modal fade" id="editModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Edit Data software </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form class="needs-validation" id="form-edit" method="post" novalidate>
                    ...
                    <div class="modal-body">
                        ...
                        @method('PUT')
                        <div class="form-floating mb-3 has-validation">
                            <input type="text" class="form-control" name="software" maxlength="255" id="nama-edit"
                                placeholder="Your Name" required>
                            <label for="floatingName">software</label>
                            <div class="invalid-feedback">
                                software harus diisi
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.hapus-data').on('click', function() {
                ...
                $('#form-hapus').attr('action', 'software/' + id);
            });
            $('body').on('click', '.edit-data', function(event) {
                event.preventDefault();
                var lab = $(this).data('lab');
                $('#selectLab').val(lab);
                console.log( $('#selectLab').val());

                var id = $(this).data('id');
                $.get('/software/' + id + '/edit', function(data) {
                    // $('#modal-student-edit').modal('show');
                    console.log(data);
                    $('#nama-edit').val(data.software.software);


                    $('#form-edit').attr('action', 'software/' + data.software.id);
                        ...
                        <tbody>
                            ...
                            @foreach ($data as $lab)
                                ...
                                <tr>
                                    ...
                                    <td > {{ $lab->nama_lab }}</td>
                                    <td>
                                        @if ($lab->software->isEmpty())
                                        <div>-</div>
                                        @endif
                                    @foreach ($lab->software as $software )
                                    <div class="d-flex justify-content-between my-2">{{ $software->software }}
                                        <span class="float-right">
                                            <button type="button" class="btn btn-success edit-data btn-sm"
                                            data-bs-target="#editModal" data-lab="{{ $lab->id }}"
                                            data-id="{{ $software->id }}" data-bs-toggle="modal"><i
                                            class="bi bi-pencil-square"></i> </button>

                                            <button type="button" class="btn btn-danger hapus-data btn-sm"
                                            data-bs-target="#hapusModal" data-id="{{ $software->id }}"
                                            data-nama="{{ $software->software }}" data-bs-toggle="modal"><i
